if Config.Framework == 'VORP' then
    ...
    local function isPlayerInDrillZone()
        if not hasDrillZones() then
            return true
        end

        return next(activeDrillZones) ~= nil
    end

    local function showDrillPrompt()
        if usingDrillTarget or drillPromptVisible or not Config.IceDrill or not Config.IceDrill.enabled then
            return
        end
    ...
    local function registerDrillTarget()
        if usingDrillTarget or not Config.IceDrill or not Config.IceDrill.enabled or not Config.IceDrill.useTarget then
            return
        end
        ...
        if GetResourceState then
            targetReady = GetResourceState('ox_target') == 'started'
        end

        if not targetReady then
            local ok = pcall(function()
                return exports.ox_target and exports.ox_target.addModel
            end)
        ...
        local maxDistance = Config.IceDrill.interactionDistance or 2.0
        local options = {
            {
                name = 'jc-mining:ice-drill',
                icon = Config.IceDrill.targetIcon or 'fa-solid fa-icicles',
                label = Config.IceDrill.targetLabel or 'Drill for ice',
        ...
        local success, id = pcall(function()
            return exports.ox_target:addModel(Config.IceDrill.prop, options)
        end)

        if success and id then
            drillTargetId = id
            usingDrillTarget = true
        end
    end
    ...
    Citizen.CreateThread(function()
        ...
        if Config.IceDrill and Config.IceDrill.enabled and hasDrillZones() then
            for index, zone in pairs(Config.IceDrill.zones) do
                if zone.coords then
                    local zoneName = zone.id or ('ice_drill_zone_' .. index)
                    local drillZone = PolyZone:Create(zone.coords, {
                        name = zoneName,
                        minZ = zone.minZ,
                        maxZ = zone.maxZ,
                    ...
                    drillZone:onPlayerInOut(function(onInside)
                        if onInside then
                            activeDrillZones[zoneName] = true
                        else
                            activeDrillZones[zoneName] = nil
        ...
        Citizen.CreateThread(function()
            ...
            while true do
                ...
                if not isWorking then
                    ...
                    if drill ~= 0 and isPlayerInDrillZone() then
                        if not usingDrillTarget then
                            sleep = 0
                            showDrillPrompt()
    ...
    RegisterNetEvent('jc-mining:client:StartIceDrilling', function()
        ...
        if not isPlayerInDrillZone() then
            VORPcore.NotifyCenter('You need to be inside a drill site.', 3000)
            return
        end
    ...
    AddEventHandler('onResourceStop', function(resource)
        ...
        if usingDrillTarget and drillTargetId then
            pcall(function()
                exports.ox_target:removeModel(drillTargetId)
            end)
            usingDrillTarget = false
            drillTargetId = nil
        end
    end)
end
